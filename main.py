...
async def handle_special_command(update, context, command):
    if command == "donation":
        ...
        await update.message.reply_text(
            ...
            Это энергии, которые живут в тебе\.\n
            Хочешь узнать, какая стихия ведёт тебя\?"",
            parse_mode="MarkdownV2",
            reply_markup=reply_markup
        )
        return True
        ...
        await update.message.reply_text(
            ...
            Ты чувствуешь, что нечто завершилось\?\n
            Тогда готовься к возрождению\.",
            parse_mode="MarkdownV2",
            reply_markup=reply_markup
        )
        return True
        ...
        await update.message.reply_text(
            "🔮 Зеркало не врёт\.\n
            Оно лишь показывает то, что ты сам избегаешь увидеть\.",
            parse_mode="MarkdownV2",
            reply_markup=reply_markup
        )
        return True
        ...
        await update.message.reply_text(
            "🌑 Тень — это часть тебя, забытая, но живая\.\n
            Посмотрим в неё вместе — и она станет силой\.",
            parse_mode="MarkdownV2",
            reply_markup=reply_markup
        )
        return True
        ...
        await update.message.reply_text(
            "💧 Очищение — это обряд освобождения от старого\.\n
            Вода, слово или дыхание могут стать началом новой главы\.",            
            parse_mode="MarkdownV2",
            reply_markup=reply_markup
        )
        return True
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            "🔮 Заклинание — это слово с намерением и силой\.\n
